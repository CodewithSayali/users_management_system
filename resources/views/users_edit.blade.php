        ...
        <form method="POST" id="user-edit-form">
            ...
            <div class="row mt-3">
                ...
                <div class="col-md-3">
                    ...
                    <input type="number" name="mobile" value="{{ old('mobile', $user->mobile) }}" class="form-control" maxlenght="10">
                </div>
                <div class="col-md-4">
                ...
                @foreach ($user->addresses as $index => $address)
                    <div class="address-block border p-3 rounded mt-3">
                        <div class="row g-3">
                            <div class="col-md-2">
                                <label>Type</label>
                                <select name="addresses[{{ $index }}][addresstype_xid]" class="form-select"
                                    required>
                            ...
                            <div class="col-md-2">
                                <label>Country</label>
                                <select name="addresses[{{ $index }}][country]" class="form-select" required>
                            ...
                            <div class="col-md-2">
                                <label>State</label>
                                <select name="addresses[{{ $index }}][state]" class="form-select" required>
                                    <option value="{{ $address->state_xid }}">
                                        {{ $address->state->name ?? 'Selected State' }}
                                    </option>
                                </select>
                            </div>
                            <div class="col-md-2">
                                <label>City</label>
                                <select name="addresses[{{ $index }}][city]" class="form-select" required>
                                    <option value="{{ $address->city_xid }}">
                                        {{ $address->city->name ?? 'Selected City' }}
                                    </option>
                                </select>
                            </div>
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        if (data.status == 200) {
                            ...
                            window.location.href ="{{ route('users.list') }}"; // Adjust route name accordingly
                        } else {
                            toastr.error(data.message || 'User updated successfully!');
                        }
                    },
